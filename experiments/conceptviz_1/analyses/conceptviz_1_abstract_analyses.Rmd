...
centroids_HK_GE <- centroids %>%
  ...
  bind_rows(data.frame(country_code ="HK", 
                       LAT = 22.28552, 
                       LONG = 114.15769)) %>%
  bind_rows(centroids)
...
all_measures_with_dyadic <- all_measures %>%
  ...
  group_by(trial_ID2) %>%
  slice(1) %>%
  select(trial_ID2, category, log_normalized_n_events_all, asjp_dist, centroid_dist_meters) 
```

Merge in country variables
```{r}
item_means2 <- item_means %>%
  left_join(all_measures_with_dyadic, by = "trial_ID2") 
...
# mixed effect model
summary(lmer(rating ~  scale(centroid_dist_meters) + (1 |trial_ID2) + (1|subj_id) + (1|category), all_measures_with_dyadic))

summary(lmer(rating ~  scale(log_normalized_n_events_all) + (1 |trial_ID2) + (1|subj_id) +(1|category), all_measures_with_dyadic))

summary(lmer(rating ~  scale(asjp_dist) + (1 |trial_ID2) + (1|subj_id) + (1|category), all_measures_with_dyadic))

summary(lmer(rating ~ scale(centroid_dist_meters) + 
               scale(log_normalized_n_events_all) + 
               scale(asjp_dist) +
               (1|trial_ID2) + (1|subj_id) + (1|category), 
