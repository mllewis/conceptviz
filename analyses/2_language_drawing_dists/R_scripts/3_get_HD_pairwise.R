...
# Loop over categories for country-country pair and write file
get_HDS_for_country_pairs <- function(country1, 
                                      country2, 
                                      categories, 
                                      inputpath, 
                                      outputpath, 
                                      py) {
  ptm <- proc.time()
  print(paste0("=== ", country1, " ", country2, " ==="))
...
### COUNTRY-COUNTRY PAIRS ###
country_country_pairs <- list(x = GOOD_COUNTRIES, y = GOOD_COUNTRIES) %>%
  cross_df() %>%
  get_unique_pairs() %>%
  as.data.frame() %>%
...
### DO THE THING ###
# loop over country-country pairs
walk2(country_country_pairs$countrycode1, 
      country_country_pairs$countrycode2, 
        get_HDS_for_country_pairs, 
        CATEGORY_NAMES, 
        INPUT_PATH, 
        OUTPUT_PATH,
        py2)