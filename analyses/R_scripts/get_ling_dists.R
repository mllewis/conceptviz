...
country_lang <- read_csv("../../data/supplementary_data/cultural_sim_measures/lang/geo_cepii.csv") %>%
  select(iso2, langoff_1) %>%
  mutate(iso2 = fct_recode(iso2,
                           "RS" = "YU")) %>%
  group_by(iso2) %>% 
  slice(1)
...
g_countries_lang <- g_countries %>%
  left_join(country_lang, by = c("country_code" = "iso2")) %>%
  rename(language = langoff_1) %>%
...
g_indices <- which(dimnames(wals_euclidean)[[1]] %in% g_countries_lang_full$iso)
...
g_indices_asjp <- which(dimnames(asjp)[[1]] %in% g_countries_lang_full$iso)
...
all_lang_dists <-  full_join(g_wals_euclidean, g_asjp) %>%
  left_join(g_countries_lang_full %>% select(country_code, iso), by = c("lang1" = "iso")) %>%
  rename(country_code1 = country_code)  %>%
  left_join(g_countries_lang_full %>% select(country_code, iso), by = c("lang2" = "iso")) %>%
  rename(country_code2 = country_code) %>%
  select(country_code1, country_code2, lang1, lang2, wals_euclidean_dist, asjp_dist)
