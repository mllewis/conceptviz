...
Read in HD data
```{r}
items <- c("line", "circle", "square", "hexagon", 
           "triangle", "octagon", "ear", "hand", 
           "knee", "mouth", "eye", "face", "foot", 
           "leg", "toe", "arm")

items <- c("hand", "knee", "mouth", "foot", 
           "leg", "toe", "arm")

pairwise_hds_raw <-  data.frame()
for (i in 1:length(items)) {
    s <- read_csv(paste0("../../data/hausdorff_similarities/pair_lists/", items[i], "_sampled_pairs_with_sims_hd.csv"),  col_types = list(col_character(), col_character(), col_double(), col_character(), col_character(), col_character())) %>%
      slice(1:2500)
...
pairwise_hds <- pairwise_hds_raw %>%
  ...
  mutate(category = ifelse(item %in% c("line", "circle", "square", "hexagon", 
           "triangle", "octagon"), "shape", "bodypart"))
...
```{r}
# LANGUAGE
summary(lmer(log_hd_sim ~ centroid_dist_meters + wals_euclidean_dist + 
                (1|all_codes) +(1|item), 
...
summary(lmer(log_hd_sim ~ asjp_dist + 
               ...
               (1|all_codes) + (1|item), 
              filter(pairwise_hds_with_predicts, category == "shape")))


summary(lmer(log_hd_sim ~ asjp_dist * category + 
               (1|all_codes)+(1|item), 
             pairwise_hds_with_predicts))
             
summary(lmer(log_hd_sim ~ asjp_dist + centroid_dist_meters +
               (1|all_codes), 
             filter(pairwise_hds_with_predicts, category == "bodypart")))
...
summary(lmer(log_hd_sim ~ asjp_dist + 
               (1|country_code_1) + (1|country_code_2) +  (1|item), 
             filter(pairwise_hds_with_predicts,  category == "shape")))


# DISTANCE
summary(lmer(log_hd_sim ~ centroid_dist_meters * category +  
               (1|country_code_1) + (1|country_code_2) +  (1|item), 
             pairwise_hds_with_predicts))
...
summary(lmer(log_hd_sim ~ centroid_dist_meters +
               (1|country_code_1) +(1|country_code_2) +  (1|item),  
             filter(pairwise_hds_with_predicts,category == "bodypart")))
...
# EVENTS
summary(lmer(log_hd_sim ~ log_normalized_n_events_all * category  + 
               (1|country_code_1) +(1|country_code_2)+  (1|item), 
             pairwise_hds_with_predicts))
...
# events + asjp
summary(lmer(log_hd_sim ~ log_normalized_n_events_all + asjp_dist+ 
               (1|country_code_1) +(1|country_code_2) +  (1|item),  
             filter(pairwise_hds_with_predicts,category == "bodypart")))
...
# Euclidean
summary(lmer(log_hd_sim ~ wals_euclidean_dist*category +
                (1|all_codes)+  (1|item), 
             pairwise_hds_with_predicts))
...
pairwise_hds_with_predicts_long <- pairwise_hds_with_predicts %>% 
    group_by(all_codes, item, category) %>%
...
pairwise_hds_with_predicts_long %>%
  ...
  do(tidy(cor.test(.$log_hd_sim, .$value, na.rm = T))) %>%
  select(-parameter, -method, -alternative) %>%
  arrange(p.value) %>%
  as.data.frame()
```
